...
inExpr (Left x) = Num x
inExpr (Right (o,(e,e'))) = Bop e o e'

outExpr :: Expr -> Either Int (Op,(Expr,Expr))
outExpr (Num x) = Left x
outExpr (Bop x op y) = Right (op,(x,y))
...
hiloExpr a b =cataExpr(b).anaExpr(a)

calcula :: Expr -> Int
calcula = cataExpr g
    where
    g :: Either Int (Op,(Int,Int)) -> Int
    g = either g1 g2
    g1 :: Int -> Int
    g1 x = x
    g2 :: (Op,(Int,Int)) -> Int
    g2 (Op op,(x,y))= case op of
        "+"->  x+y
        "-"->  x-y
        "*"->  x*y
...
compile = cataExpr g . read
    where
    g :: Either Int (Op,(Codigo,Codigo)) -> Codigo
    g (Left x)= ["Push " ++ show x]
    g (Right (Op op,(x,y)))=x ++ y ++ [(case op of "+" -> "ADD";"*" -> "MUL";"-" -> "SUB")]
...
inL2D (Left x)=  (Unid x)
inL2D (Right(y,(a,a')))=Comp y a a'


outL2D :: X a b -> Either a (b, (X a b,X a b))
outL2D (Unid c)= Left c
outL2D (Comp y a a')=Right (y, (a ,a'))
...
dimen =  cataL2D g
    ...
    g (Left x)= (fromIntegral(fst(fst x)),fromIntegral(snd(fst x)))
    g (Right (y,((a,b),(a',b'))))=(a+a',b+b')

calc :: Tipo -> Origem -> (Float, Float) -> Origem
calc a b (c,d)=(case a of V -> (fst b+c/2,snd b + d);
                          Vd -> (fst b+c,snd b + d);
                          Ve -> (fst b,d + snd b);
                          H -> (fst b + c,d/2 + snd b);
                          Ht -> (fst b+c,snd b + d);
...
calcOrigins = anaL2D g
    ...
    g (a,b)= case outL2D a of
        Left x -> Left (x,b)
        Right (t,(i,i')) -> Right((),((i,b),(i',calc t b (dimen i))))
...
caixasAndOrigin2Pict = G.Pictures . map (\(o,((w,h),(t,c))) -> crCaixa o (fromIntegral w) (fromIntegral h) t c) . agrup_caixas . calcOrigins
\end{code}
...
cos' x = prj . for loop init where
   loop (c,h,f) = (c+h, h*f,(1/2)*f*(1/(1/3)))
   init = (1,-(x^2)/2,1/3)
   prj (c,h,f) = c
\end{code}
