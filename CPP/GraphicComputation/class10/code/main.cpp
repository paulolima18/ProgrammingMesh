...
void computeNormal(int i, int j) {
	...
	//Modulo do vector
	modulo = sqrtf(powf(a, 2) + powf(b, 2) + powf(c, 2));

	// fill the normal vector with the normal for vertex at grid location (i,j)
	normal.push_back(a / modulo);
	normal.push_back(b / modulo);
	normal.push_back(c / modulo);
}
...
void renderTerrain() {
	...
	float white[4]={1.0f, 1.0f, 1.0f,1};
