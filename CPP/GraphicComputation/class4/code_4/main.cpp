...
void cylinder2(float radius, float height, int sides) {
	...
	step = 360.0 / sides;
    float _temp = sides*9+sides*18+sides*9;
    vertexB = (float*)malloc(sizeof(float) * (_temp*3) );
    ...
    glBufferData(GL_ARRAY_BUFFER,sizeof(float) * _temp, vertexB, GL_STATIC_DRAW);
}

void draw(int sides) {
    int count = sides*3+sides*6+sides*3;
...
void renderScene(void) {
	...
	glutSwapBuffers();
    char title[1024];
    sprintf(title, "SIDES: %d | FPS: %4f", sides, fps_counter());
